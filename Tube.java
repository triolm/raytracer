...
public class Tube extends Surface {
    ...
    //Minimum distance for a valid collision. This prevents the surface's rays from colliding with itself.
    ...
    public Intersection intersect(Ray ray){
        //Based on https://math.stackexchange.com/questions/3248356/calculating-ray-cylinder-intersection-points
        ...
        //The x distance of the ray is constant because the x axis is defined by the ray being
        //  perpendicular to it. So, project into x to quickly eliminate rays that don't get close.
        ...
        //Then, project into the new frame's axes. As mentioned above, the ray's x is constant, so dx = 0.
        //  rayDir.dot(frameX) always returns 0.
        Vector rayDir = ray.getDirection().normalize();
        Vector rayMoveInFrame = new Vector(
            0, rayDir.dot(frameY), rayDir.dot(frameZ)
        ).normalize();
        ...
        //we don't care about dz yet because the cylinder is infinite for our purposes right now
        ...
        //This should have already been covered by the shortest-distance check above but it's cheap to double-check.
        if (determinant < 0){
            return null;
        }
        ...
        if (distanceInFrame < -EPSILON){
            //Specific check for being inside of a cylinder (first solution would be behind you)
            distanceInFrame = ((-b)+Math.sqrt(determinant))/(2*a);
        }
        
        //If both solutions are behind you, or you're colliding with yourself, no collision.
        if (distanceInFrame < EPSILON){
            return null;
        }
        ...
        if(collisionInFrame.getZ() < 0 || collisionInFrame.getZ() > vertex2.subtract(vertex1).length()){
            //If the collision doesn't fit between those, check for a secondary collision "through" an endcap.
            //  (e.g. looking up from underneath: first collision is z<0, but second collision would be z>0.)
            distanceInFrame = ((-b)+Math.sqrt(determinant))/(2*a);
            collisionInFrame = rayInFrame.evaluate(distanceInFrame);
            if(collisionInFrame.getZ() < 0 || collisionInFrame.getZ() > vertex2.subtract(vertex1).length()){
                //If the secondary collision isn't in the right z range either, you're looking above, below, or through.
                return null;
            }
        }
        
        //Now that we have the distance, we can generate the return value based off the original ray.
        //  (This works because distance scaling is the same in both reference frames.)
        Point collision = ray.evaluate(distanceInFrame);
        
        //To calculate the normal, we need the position at the same height in the center of the cylinder.
        //  Start with vertex1 (base of the cylinder) and apply movement using the in-frame Z coords.
        ...
        //If this is a collision with the inside of the tube, make sure the normal points inward as well
